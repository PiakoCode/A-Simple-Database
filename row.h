...
/**
 * @brief 文件结构体
 * 定义了文件在磁盘上的数据结构和操作。Pager用于管理文件级别的IO操作。
 */
typedef struct
{
    int file_descriptor;          // 文件描述符
    uint32_t file_length;         // 文件的大小
    uint32_t num_pages;           // 文件包含的页数
    void *pages[TABLE_MAX_PAGES]; // 指向页内存空间的指针数组
} Pager;
...
typedef struct
{
    ...
    uint32_t cell_num; // 当前单元号
    bool end_of_table; // 是否到达表的尽头
} Cursor;
...
const uint32_t LEAF_NODE_NEXT_LEAF_OFFSET = LEAF_NODE_NUM_CELLS_SIZE + LEAF_NODE_NUM_CELLS_SIZE;
...
// 打开数据库文件、关闭数据库文件、打开Pager、获取页面、冲刷页面到磁盘的函数
...
// 与节点操作相关的辅助函数，比如获取节点类型、判断是否为根节点等
...
void internal_node_insert(Table *table, uint32_t parent_page_num, uint32_t child_page_num)
{
    ...
    *internal_node_num_keys(parent) = original_num_keys + 1;
    ...
    else
    {
        ...
        {
            void *destinaion = internal_node_cell(parent, i);
            void *source = internal_node_cell(parent, i - 1);
            memcpy(destinaion, source, INTERNAL_NODE_CELL_SIZE);
        }
